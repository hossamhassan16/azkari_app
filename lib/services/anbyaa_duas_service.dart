import 'dart:convert';
import 'package:azkari_app/models/anbyaa_duas_model.dart';
import 'package:flutter/services.dart';
import 'package:shared_preferences/shared_preferences.dart';

class AnbyaaDuasService {
  static final AnbyaaDuasService _instance = AnbyaaDuasService._internal();
  factory AnbyaaDuasService() => _instance;
  AnbyaaDuasService._internal();
  List<AnbyaaDuaModel>? _cachedDuas;
  ...
  Future<List<AnbyaaDuaModel>> loadAnbyaaDuas() async {
    if (_cachedDuas != null) {
      return _cachedDuas!;
    }
    ...
    try {
      final String jsonString =
          await rootBundle.loadString('assets/data/duas_anbyaa.json');
      final Map<String, dynamic> data = json.decode(jsonString);

      final List<dynamic> duasList = data['duas_anbyaa'] as List<dynamic>;
      _cachedDuas = duasList
          .asMap()
          .entries
          .map((entry) =>
              AnbyaaDuaModel.fromJson(entry.value.toString(), entry.key))
          .toList();
  ...
  Future<void> _loadProgress() async {
    ...
    for (int i = 0; i < _cachedDuas!.length; i++) {
      ...
      final key = 'anbyaa_dua_progress_${dua.id}';
  ...
  Future<void> saveProgress(String duaId, int count) async {
    final key = 'anbyaa_dua_progress_$duaId';
  ...
  Future<List<AnbyaaDuaModel>> getMyAzkar() async {
    ...
    return duasList
        .map((duaJson) => AnbyaaDuaModel.fromSavedJson(duaJson))
        .toList();
  }

  // Add dua to "My Azkar"
  Future<void> addToMyAzkar(AnbyaaDuaModel dua) async {

...
class TasbehService {
  ...
  Future<void> _loadProgress() async {
    ...
    for (int i = 0; i < _cachedDuas!.length; i++) {
      ...
      final key = 'tasbeh_progress_${dua.id}';
  ...
  Future<void> saveProgress(String duaId, int count) async {
    final key = 'tasbeh_progress_$duaId';
  ...
  Future<List<TasbehModel>> getMyAzkar() async {
    ...
    return duasList
        .map((duaJson) => TasbehModel.fromSavedJson(duaJson))
        .toList();
  }

  // Add dua to "My Azkar"
  Future<void> addToMyAzkar(TasbehModel dua) async {

...
class IstighfarService {
  static final IstighfarService _instance = IstighfarService._internal();
  factory IstighfarService() => _instance;
  IstighfarService._internal();

  List<IstighfarModel>? _cachedDuas;
  ...
  Future<List<IstighfarModel>> loadIstighfarDuas() async {
    if (_cachedDuas != null) {
      return _cachedDuas!;
    }
    ...
    try {
      final String jsonString =
          await rootBundle.loadString('assets/data/istighfar_duas.json');
      ...
      _cachedDuas = duasList
          ...
          .entries
          .map((entry) =>
              IstighfarModel.fromJson(entry.value.toString(), entry.key))
          .toList();
  ...
  Future<void> _loadProgress() async {
    ...
    for (int i = 0; i < _cachedDuas!.length; i++) {
      ...
      final key = 'istighfar_progress_${dua.id}';
  ...
  Future<void> saveProgress(String duaId, int count) async {
    final key = 'istighfar_progress_$duaId';
  ...
  Future<List<IstighfarModel>> getMyAzkar() async {
    ...
    return duasList
        .map((duaJson) => IstighfarModel.fromSavedJson(duaJson))
        .toList();
  }

  // Add dua to "My Azkar"
  Future<void> addToMyAzkar(IstighfarModel dua) async {

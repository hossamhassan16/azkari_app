import 'dart:convert';
import 'package:azkari_app/models/hamd_model.dart';
import 'package:flutter/services.dart';
import 'package:shared_preferences/shared_preferences.dart';

class HamdService {
  static final HamdService _instance = HamdService._internal();
  factory HamdService() => _instance;
  HamdService._internal();

  List<HamdModel>? _cachedDuas;
  ...
  Future<List<HamdModel>> loadHamdDuas() async {
    if (_cachedDuas != null) {
      return _cachedDuas!;
    }
    ...
    try {
      final String jsonString =
          await rootBundle.loadString('assets/data/hamd_duas.json');
      ...
      _cachedDuas = duasList
          .asMap()
          .entries
          .map((entry) => HamdModel.fromJson(entry.value.toString(), entry.key))
          .toList();
  ...
  Future<void> _loadProgress() async {
    ...
    for (int i = 0; i < _cachedDuas!.length; i++) {
      ...
      final key = 'hamd_progress_${dua.id}';
  ...
  Future<void> saveProgress(String duaId, int count) async {
    final key = 'hamd_progress_$duaId';
  ...
  Future<List<HamdModel>> getMyAzkar() async {
    ...
    return duasList.map((duaJson) => HamdModel.fromSavedJson(duaJson)).toList();
  }

  // Add dua to "My Azkar"
  Future<void> addToMyAzkar(HamdModel dua) async {
